...
task4:
# move save button to AI tools bar at the end, it can save essay and AI outpots to supabase.
# Add input start button to original place of save, it validate the user has wriqteness, if not, it will prompt user to write.
# local cache the essay and AI outputs for each AI tools button, only call AI at the first time that user click the button, if user click the button again, it will use cache.
